...
**Answer 6:**
I went onto yahoo and used the search term "How to calculate a rolling average in R". I decided to use the top search result. This took me to statology.org . I chose to do a k of 5 as the next question first mentions a period of five years. I did not have to change the code much in order to answer the question successfully, I just had to add in installation of the packages and substitute in my own data.(Wuld not allow me to render with package installation code added in. I will write the code I had said below)
...
When all plotted on the same plot, you can see that they all follow the same general path but some of the averages are much more varying/robust and others are more smooth. This is because the different averages have different levels of smoothing of data by averaging shorter or longer amounts of time, with the shorter periods having more variation.
...
**Answer 9:**
The 2013 IPCC Report reported the pause as beginning in 1998, and that it showed a much smaller increasing trend in global mean temperature than previously observed. By looking at the rolling averages and charts I made, I do not find evidence for such a pause. If you look at the chart which shows change in global mean temperature over the years, you can see that around 1998, the change in global mean temperature is still increasing. As well, it is actually increasing at rates higher than shown in the early 1900s. Looking at the chart you can see more fluctuation within temperature earlier in the chart until reaching a turning point around 1960, in which a consistent increasing trend begins until the end of the chart. Overall, I do not find evidence for such a pause.
...
The data set is Historical CO2 Record from the Vostok Ice Core. It has four columns, first is depth which is measured in meters. Next is age of the ice which is measured in years before present. Next column is mean age of the air which is also measured in years before present. The last column is co2 concentration which is measured in parts per million by volume. The data is historic and has a larger window of time in between data measurements, which means it has a lower temporal resolution than other data sets such as the Mauna Loa dataset. I can conclude that recent atmospheric CO2 levels have seen a significant increase, to an almost unnatural degree. In the chart, you can see fluctuation of atmospheric CO2 levels over a long period of time, with natural increases and decreases. You can also see that it is generally balancing itself out, with no significant increases. This is until reaching recent years. Recent data shows an extreme and very rapid increase in atmospheric CO2 levels. This continues increasing until reaching the end of the chart. 
**Answer 10:**

```{r}
vostok <- read_table("vostok.icecore.co2",
                     col_names = c("Depth_m", "Age_of_ice_yearBP", "Mean_age_of_air_yearBP", "CO2_concentration_ppmv"), 
                     comment = "*")

vostok <- vostok[-c(1:4),]

class("Depth_m")
#just to check and make sure I need to transform data

vostok <- transform(vostok, Depth_m = as.numeric(Depth_m), 
                    Age_of_ice_yearBP = as.numeric(Age_of_ice_yearBP),
                    Mean_age_of_air_yearBP = as.numeric(Mean_age_of_air_yearBP),
                    CO2_concentration_ppmv = as.numeric(CO2_concentration_ppmv))

vostok <- vostok |> 
  mutate(Calendar_year = 1950 - as.numeric(Age_of_ice_yearBP))
...
vostok |> 
  ggplot(aes(Calendar_year, CO2_concentration_ppmv)) +
           geom_line()+
           theme_bw()+
           labs (x = "Calendar Year", y = "CO2 Concentration(ppmv)")

co2_rename <- co2 |> 
  rename(CO2_concentration_ppm = monthly_average)

vostok2 <- vostok |> 
  rename(year = Calendar_year, CO2_concentration_ppm = CO2_concentration_ppmv)

combined_data <- bind_rows(vostok2, co2_rename) |> 
  arrange(year)

combined_data |> 
  ggplot(aes(year, CO2_concentration_ppm)) +
  geom_line() +
  theme_bw() +
  labs(title = "Historic and Current Atmospheric CO2 Levels Over Time", x = "Year", y = "CO2 Concentration(ppmv)")
...
I worked with Hailey Trittin on question 4 and question 10. For question four they worked with me for the transformation portion and they helped me with the same transformation piece in question 10. I used statology.org in order to complete question 6. I then used what I learned in question 6 in order to answer question 7. I used chatgpt on question four, giving it the prompt of "in R, how do I skip the top 4 lines of code without using the skip() function?" and it taught me to use -c(). 
